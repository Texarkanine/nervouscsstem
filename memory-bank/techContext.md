...
- **uv + ProperDocs** — docs site. `uv sync --group docs` (lockfile `uv.lock`, config `properdocs.yml`). Relock with `uv lock --no-config --default-index https://pypi.org/simple --upgrade` so a user-level extra index (this machine: PyTorch CUDA in `~/.config/uv/uv.toml`) cannot enter the lock. Nav is the `docs/` tree plus `docs/.pages` (`mkdocs-awesome-pages-plugin`); `properdocs.yml` has no `nav:`. `not_in_nav` hides `reading.md` and `boards/**`. Swatch boards are static HTML in `docs/boards/` — `extra_css` is not injected into them. Dual-load of `nerv.css` / `nerv.js` is [`scripts/resolve-docs-assets.mjs`](../scripts/resolve-docs-assets.mjs): `--mode local` copies `dist/`, `--mode cdn` writes jsDelivr stand-ins.
