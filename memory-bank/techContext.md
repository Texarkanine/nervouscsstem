...
- Docs site: `npm run docs:serve` / `docs:build`. PR builds use local mode; the `publish-pages` job on `release-please.yaml` uses CDN mode after `publish-npm`.
...
Automated checks via Node.js built-in test runner (`node --test`) in `test/`. Stylelint enforces `.nerv-` prefix convention and is configured in `.stylelintrc.json`. Test and lint commands are defined in `package.json`. The npm tarball must contain `dist/nerv.css` and `dist/nerv.js` from `dist/` and not other dist artifacts; that contract is `test/publish-contract.test.mjs`. The docs local-vs-CDN load path is `test/docs-assets.test.mjs`.
