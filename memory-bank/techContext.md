...
- `npm run build:offline` — builds `dist/nervouscsstem-offline.zip` (GitHub Release asset, never in the npm tarball) via `scripts/build-offline-bundle.mjs`: `nerv.css` with font URLs rewritten to `fonts/`, `nerv.js`, fonts + OFL texts from exact-pinned `@fontsource*` devDependencies, AGPL `LICENSE`, generated `README.md` notices, `manifest.json` hashes. Zip writer is `fflate` with a fixed local-field mtime (byte-reproducible). Faces map to fontsource files by `unicode-range` equality; unmappable CSS fails the build. Maintainer notes: `docs/service-manual.md` "Offline bundle fonts". The release job builds it before `npm publish`; the PR docs build builds it too.
