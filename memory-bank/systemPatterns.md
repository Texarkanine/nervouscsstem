...
The documentation site is the teaching surface, authored only under `docs/`. Five `docs/boards/*.html` files are published void swatches adapted from matching `ref/` pages; they may drift. `ref/` stays the fixture. Do not list boards as MkDocs `extra_templates` (that Jinja-processes them into Material). Material docs chrome must not call `NERV.init()`. Each catalog example is one `html island` fence: the superfences formatter in `scripts/nervouscsstem_docs/` renders the `.nerv-docs-island` (body minus `<script>`) and the highlighted copy from that one body at build time. Island chrome is fence options (`init="…"` → `data-nerv-init`, `state="…"` → `nerv-state-*` on the wrapper), never in the body, and JS recipes `querySelector` a class present in that markup. Scanlines is the one plain `html` fence (viewport overlay). `skills/nerv/` is a placeholder `SKILL.md` until selected pages are promoted into the skill; do not lockstep-copy this tree.
