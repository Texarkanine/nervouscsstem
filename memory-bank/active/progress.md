...
- **2026-03-29**: Rework-3 build (via /refresh systematic diagnosis). Persistent ARM R → UNIT-01 overlap in angled-up lists. Root cause: geometric asymmetry (angled-up shapes extend toward siblings, angled-down extend away). Attempted direction-specific margin-bottom overrides — multiplier 7 eliminated overlap but created enormous visual canyon. Font-size test confirmed fundamental fragility.
- **2026-03-29**: Rotated nesting ABANDONED. CSS transforms don't affect layout — no pure-CSS solution can robustly bridge the layout/visual disconnect across font sizes and content widths. All rotation × nesting rules removed from `_list.scss`. Tests B40, B41, B46, B48 removed. Rotated nesting demo removed from `ref/ref-lists.html`. 353/353 tests pass. Archived to `memory-bank/archive/enhancements/20260329-nerv-phase7-m5-rework.md` with full research documentation.
