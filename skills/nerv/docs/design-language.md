...
A comprehensive catalog of recurring visual design patterns, compositional strategies, and aesthetic themes from the Future User Interfaces (FUI) of *Neon Genesis Evangelion* (TV series, episodes 1–26) and *The End of Evangelion*. Where the [Atomic Elements](atomic-elements.md) catalog lists the specific building blocks you'd implement, this document describes the *compositions, vibes, and thematic treatments* — the large-scale visual language that makes a NERV interface feel like a NERV interface.

Sourced from 271 reference frames. Reference album: [Evangelion User Interfaces (Images)](https://imgur.com/a/mvW7g)

---

## 1. Hazard Chevron Striping

Diagonal warning stripes — usually in red/black, red/darker-red, or orange/black — running at ~45° angles above and below critical text or along the borders of danger zones. Directly descended from real-world industrial hazard marking (ISO 3864), but stylized with heavier stroke weight and more saturated color. Often paired with full-bleed status cards (see §2). The chevrons create a visual "do not cross" frame that draws the eye inward to the enclosed content.
...
## 3. Bilingual Japanese/English Labeling
...
## 4. Green Wireframe Topographic Map
...
## 7. Waveform / Oscillograph Display
...
Note: the specific Psychographic / Sync Curve Trace is cataloged as an atomic element in the [companion doc](atomic-elements.md). This section covers the broader oscillograph aesthetic.
...
## 10. Biometric Targeting Overlay
...
## 12. System Architecture / Code Visualization
...
Includes specialized variants: the Japanese naval engine order telegraph (前進/停止/後進 with speed grades 微速力 through 強速力) in the Pacific Fleet scenes, and the GEHIRN-branded PDA (palm-sized device with amber-on-black data cards showing personnel records or MAGI voting status in miniature).
...
## 16. Environmental Signage and Wayfinding
...
## 17. Color-Coded Multi-Monitor Command Center
...
## 18. Red Monochrome Emergency State
...
## 20. Perspective-Skewed Axis Grid
...
## 22. Angel Genetic Analysis Screen
...
## 23. Geofront Armor Layer Descent Display
...
## 25. Armor Layer Breach Progress Display
...
## 28. Entity Silhouette on Tactical Map

An orange or red-filled silhouette of a ship, Eva unit, or Angel, rendered as a detailed top-down or side profile, overlaid on the green wireframe city/terrain map (see §4). Each silhouette has a red cartouche label identifying it (DDG-173 KONGO, UN CVN075 MPACN Over The Rainbow, 6th ANGEL WITH EVA-02). The silhouettes are transparent enough to show map detail beneath. This is the operating schematic (作戦行動予定図) view — the compositional pattern of placing identified entities onto a spatial reference.
...
## 29. Eva Damage Status Display

Amber-on-black schematic of an Eva unit with a numbered list of OPERATING PARTS down both sides (0001–0035+). Each part slot shows either a green OPERATING cartouche or a red DAMAGED cartouche. An entry plug schematic appears in a sub-panel. The Eva silhouette is centered with damage overlays. This is the mech maintenance readout — the most mechanically detailed Eva interface in the series, composing body silhouettes, status cartouches, and segmented indicators into a single dense diagnostic view.
...
## 31. Energy Observational Data Display
...
## 32. 666 Hex Pattern / MAGI Defense Visualization
...
## 33. Reserve Energy Depletion Staircase
...
## 34. Solenoid Graph Pattern

Paired 3D wireframe trace patterns — one green (FIELD NEGATIVE), one red (FIELD POSITIVE) — showing helical/toroidal shapes rendered as line meshes on calibrated axes. The traces look like three-dimensional magnetic field visualizations or solenoid cross-sections. Used during Instrumentality/Third Impact sequences to represent AT field interaction data. Labeled SOLENOID GRAPH PATTERN A. The visual makes something fundamentally metaphysical — the interaction between two souls' AT fields — feel like it could be graphed on an oscilloscope. That pseudoscientific authority is the point.
...
## 35. Bracket/Tree Cascade Diagram

Columns of segmented bars connected by branching lines that merge and split — resembling a circuit diagram or tournament bracket turned vertical. The segments shift from black (inactive/nominal) to red (active/critical) as a state change propagates through the system. The connective lines show dependency relationships between segments. Used in the Operation Yashima power distribution display. The visual effect isn't about reading which segment is active — it's about watching power cascade through a web of dependencies, each node lighting up in sequence. The complexity feels earned and systemic.
...
NERV uses two distinct terminal color schemes that map to functional domains:
- **Amber-on-black** (the dominant palette): orange/amber text and line graphics on pure black, borrowing from amber phosphor CRT monitors standard in 1980s–90s industrial and military computing. Used for *operations and command* — the bridge crew's domain. The amber ranges from deep orange (warning/critical) to bright yellow-orange (nominal). The black background bleeds into the command center darkness, making data appear to float.
- **Green-on-black** (Maya's terminals): referencing 1970s–80s phosphor terminal aesthetics (VT100, IBM 3278). Used for *programming and analysis* — Ritsuko and Maya's domain. These terminals show source code (C-like function signatures: GetStringWork, MyString, LenxCheck), small embedded graphs, and structured data layouts.

The split is functional worldbuilding: amber = operating the system, green = building/debugging the system.
...
### Color-Coded Pilot Identity
Consistent color mapping across the entire series: FIRST.C / Rei Ayanami = green, THIRD.C / Shinji Ikari = cyan/blue, SECOND.C / S. Asuka Langley = red. This mapping is applied to header labels, designation numbers (00/01/02), and panel border tints. The colors never swap. They are a persistent visual identifier across all UI contexts — test plug monitors, sync tests, pilot cards, harmonics displays. When Kaworu appears as FIFTH C., he gets amber, breaking the RGB triad. This isn't a component you build; it's a rule you apply to every component that references a pilot.
