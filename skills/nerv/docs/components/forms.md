...
CRT-console styling on ordinary form controls. Each control is shown in isolation. Filler values stay the same where a control has a value.
...
**Spec:** `.nerv-checkbox` and `.nerv-radio` use `appearance: none`. Checked+focus shadows are written as one compound selector so they do not overwrite each other.
...
`.nerv-form-{name}` on any of the above. One example.
